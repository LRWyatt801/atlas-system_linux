...
def get_args():
    ...
    """
    if len(sys.argv) < 3:
        print(f"USAGE: : read_write_heap.py <pid> \
                <search_str> <replace_str:optional>")
        exit(1)
...
def replace_heap_str(
    ...
    Args:
        ...
        replace_str (str, optional): String to be used to replace.
                                     Defaults to None or ""(empty str).
    """
    if pid is None or strt_adrs is None or end_adrs is None or \
       search_str is None:
        exit(1)
    ...
    try:
        ...
        print(f"{mem_file}: Permission Denied, \
